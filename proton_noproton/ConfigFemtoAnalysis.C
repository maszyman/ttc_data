        ...
        if (runch[ichg]) {
          ...
          for (int iepvzero = 0; iepvzero < numOfEPvzero; iepvzero++) {
            if (runepvzero[iepvzero]) {
              ...
              dtc2etaphitpc[aniter]->SetLeastProbableProton();
              ...
              // if (ichg == 2){
              cutPass2PIDetaphitpc[aniter] = new AliFemtoCutMonitorParticlePID(Form("cutPass2%stpcM%iPsi%i", chrgs[ichg], imult, iepvzero),2);//0-pion,1-kaon,2-proton
              cutFail2PIDetaphitpc[aniter] = new AliFemtoCutMonitorParticlePID(Form("cutFail2%stpcM%iPsi%i", chrgs[ichg], imult, iepvzero),2);
